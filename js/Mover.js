...
// MOVER CLASS
//Faoruq is telling you to understand this as a class not vector or any other oop it's all objects
function Mover(m, vel, loc, id, suffix, mesh, radius, colorHex) {
    ...
    this.attract = function (m) { // m => other Mover object
		...
		// Limiting the distance to eliminate "extreme" results for very close or very far objects
          //انتبه هون مجرد تعديل للقوة من دون الاهتمام بالمسافة
    ...
    this.applyForce = function (force) {
        if (!this.mass) this.mass = 1.0; // if not a mass get a udpate
        var f = force.divideScalar(this.mass); //F = m*a => a = F/m
        this.acceleration.add(f);
    };

    this.update = function () {
        //this is for adding acceleratoin then to the velocity
    ...
    this.display = function () {
        ...
        if (this.alive) {
            ...
            //this.selectionLight.intensity = 0.5; // ME ! 
        }
...
// TRAIL LINES --------------- توابع فقط للتبّع القمر
